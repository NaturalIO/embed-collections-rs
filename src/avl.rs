    ...
    pub fn nearest<'a>(
        ...
        if !current.node.is_null() {
            ...
            if let Some(node) = self.walk_dir(current.node, direction) {
                return unsafe { AvlSearchResult::new(node.as_ptr(), None) };
            }
        }
        return AvlSearchResult::default();
    }
...
mod tests {
    ...
    fn int_avl_tree_insert_here() {
        ...
        drop(rs); // Release borrow

        // Insert 5 before 10
        let here = unsafe { AvlSearchResult::new(node1_ptr, None) };
        ...
        let here = unsafe { AvlSearchResult::new(node5_ptr, None) };
