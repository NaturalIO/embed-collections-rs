...
/// Test: Borrowing space from left internal node sibling during split propagation
///
/// Tree structure before insert:
///   root(h=2) -> [inter_left | sep_key | inter_right]
///   inter_left(h=1) -> [leaf_0 | key | leaf_1]  (not full, has space)
///   inter_right(h=1) -> [leaf_2 | key | leaf_3] (full)
///
/// Where leaf_2 is full and will split on insert. The split propagates to
/// inter_right which is also full. Instead of splitting inter_right, we should
/// try to borrow from inter_left first.
///
/// This test verifies that when an internal node is full during split propagation,
/// it tries to borrow from left sibling before splitting.
#[test]
fn test_inter_borrow_from_left_sibling() {
    unsafe {
        let leaf_cap = LeafNode::<i32, i32>::cap();
        let inter_cap = InterNode::<i32, i32>::cap();

        // Create leaves for inter_left (leaf_0 and leaf_1)
        let mut leaf_0 = LeafNode::<i32, i32>::alloc();
        let mut leaf_1 = LeafNode::<i32, i32>::alloc();

        // Fill leaf_0 to min_count (half capacity) - leave space for borrowing
        let min_count = (leaf_cap + 1) / 2;
        for i in 0..min_count {
            leaf_0.insert_no_split(i as i32 * 2, i as i32 * 10);
        }

        // Fill leaf_1 similarly
        for i in 0..min_count {
            leaf_1.insert_no_split((min_count + i) as i32 * 2, (min_count + i) as i32 * 10);
        }

        // Link leaves for inter_left
        (*leaf_0.brothers()).next = leaf_1.get_ptr();
        (*leaf_1.brothers()).prev = leaf_0.get_ptr();

        // Create inter_left with space (only 1 key, capacity is higher)
        let mut inter_left = InterNode::<i32, i32>::alloc(1);
        inter_left.set_left_ptr(leaf_0.get_ptr());
        let leaf_1_first = leaf_1.get_keys()[0];
        inter_left.insert_no_split(leaf_1_first, leaf_1.get_ptr());
        // inter_left now has 1 key, can accept more

        // Create leaves for inter_right (leaf_2 and leaf_3)
        let mut leaf_2 = LeafNode::<i32, i32>::alloc();
        let mut leaf_3 = LeafNode::<i32, i32>::alloc();

        // Fill leaf_2 to capacity (full)
        for i in 0..leaf_cap {
            leaf_2.insert_no_split((100 + i) as i32 * 2, (100 + i) as i32 * 10);
        }

        // Fill leaf_3 to capacity (full)
        for i in 0..leaf_cap {
            leaf_3
                .insert_no_split((100 + leaf_cap + i) as i32 * 2, (100 + leaf_cap + i) as i32 * 10);
        }

        // Link leaves for inter_right
        (*leaf_1.brothers()).next = leaf_2.get_ptr();
        (*leaf_2.brothers()).prev = leaf_1.get_ptr();
        (*leaf_2.brothers()).next = leaf_3.get_ptr();
        (*leaf_3.brothers()).prev = leaf_2.get_ptr();

        // Create inter_right with 1 key (but leaves are full)
        let mut inter_right = InterNode::<i32, i32>::alloc(1);
        inter_right.set_left_ptr(leaf_2.get_ptr());
        let leaf_3_first = leaf_3.get_keys()[0];
        inter_right.insert_no_split(leaf_3_first, leaf_3.get_ptr());

        // Fill inter_right to capacity with more keys pointing to dummy leaves
        // We'll create some dummy leaves just to fill the internal node
        let mut dummy_leaves: Vec<LeafNode<i32, i32>> = Vec::new();
        for i in 0..(inter_cap - 1) as usize {
            let mut dummy = LeafNode::<i32, i32>::alloc();
            // Insert one key to make it valid
            dummy.insert_no_split((200 + i as i32) * 2, (200 + i as i32) * 10);
            dummy_leaves.push(dummy);
        }

        // Now add keys to inter_right
        for (i, dummy) in dummy_leaves.iter().enumerate() {
            let key = (200 + i as i32) * 2;
            inter_right.insert_no_split(key, dummy.get_ptr());
        }

        // Record original separator key between inter_left and inter_right
        let original_sep_key = leaf_2.get_keys()[0];

        // Create root
        let mut root = InterNode::<i32, i32>::alloc(2);
        root.set_left_ptr(inter_left.get_ptr());
        root.insert_no_split(original_sep_key, inter_right.get_ptr());

        // Create BTreeMap
        let total_elements = (min_count * 2 + leaf_cap * 2 + (inter_cap - 1)) as usize;
        let mut map = BTreeMap::<i32, i32> {
            root: Some(Node::Inter(root)),
            len: total_elements,
            cache: UnsafeCell::new(PathCache::new()),
        ...
        // Now insert a key that will go to leaf_2 and trigger split
        // leaf_2 is full, so it will split and propagate to inter_right
        // inter_right is also full, so it should try to borrow from inter_left
        let insert_key = 100 * 2 + 1; // Between leaf_2's first and second key
        let insert_value = insert_key * 10;
        ...
        // The tree structure should have been updated
        // Verify root still has valid structure
        let root_ref = map.get_root_unwrap().as_inter();
        assert!(root_ref.key_count() >= 1, "Root should have at least one key");

        // The separator key should have changed if borrowing occurred
        let new_sep_key = root_ref.get_keys()[0];
        println!("Original sep key: {}, New sep key: {}", original_sep_key, new_sep_key);

        // Verify tree is still valid
        assert_eq!(map.height(), 3, "Height should remain 3 after borrowing");

        // Cleanup - deallocate dummy leaves first
        drop(dummy_leaves);
        drop(map);
    }
}
