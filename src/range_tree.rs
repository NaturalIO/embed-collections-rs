...
impl<T: RangeTreeOps> RangeTree<T> {
    ...
    pub fn remove(&mut self, start: u64, size: u64) -> bool {
        ...
        let result = self.root.find(&search_rs, range_tree_segment_cmp).get_exact();
        if result.is_none() {
            return false;
        }
        ...
        if left_over && right_over {
            ...
            let s = unsafe { AvlSearchResult::new(Arc::as_ptr(&rs_node), None) };
            unsafe { self.root.insert_here(new_rs, s, AvlDirection::Right) };
        } else if left_over {
            // Remove Right end
